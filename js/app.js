...
class UI {
   ...
   imprimirCitas({citas}) { // Se puede aplicar destructuring desde la función...
        ...
        citas.forEach(cita => {
            const {titulo, fecha, hora, descripcion, id } = cita;
            ...
            descripcionParrafo.innerHTML = `<span class="font-weight-bolder">Descripción: </span> ${descripcion}`;

            // Agregar un botón de eliminar...
            const btnEliminar = document.createElement('button');
            btnEliminar.onclick = () => eliminarCita(id); // añade la opción de eliminar
            btnEliminar.classList.add('btn', 'btn-dark', 'mr-2');
   ...
   textoHeading(citas) {
        if(citas.length > 0 ) {
            ...
            heading.textContent = 'No hay tareas, comienza creando una'
        }
    }
...
function nuevaCita(e) {
    ...
    const {titulo, fecha, hora, descripcion } = citaObj;
...
function cargarEdicion(cita) {
    ...
    const {titulo, propietario, fecha, hora, descripcion, id } = cita;
